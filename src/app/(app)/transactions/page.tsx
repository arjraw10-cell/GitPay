        ...
        {txs.length > 0 && (() => {
          ...
          return (
            <div style={{ display: "flex", gap: "12px", marginBottom: "24px", maxWidth: "800px" }}>
              {[
                { label: "Total Paid", value: `${total} SOL` },
                { label: "Payouts", value: String(txs.length) },
                { label: "Avg Payout", value: `${avg} SOL` },
              ].map((s) => (
                <div key={s.label} style={{ flex: 1, border: "1px solid #e4e4e7", borderRadius: "10px", padding: "14px 18px", background: "#fff" }}>
                  <div style={{ fontSize: "11px", color: "#71717a", fontWeight: 500, marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.05em" }}>{s.label}</div>
                  <div style={{ fontSize: "20px", fontWeight: 600, color: "#000" }}>{s.value}</div>
                </div>
              ))}
            </div>
          );
